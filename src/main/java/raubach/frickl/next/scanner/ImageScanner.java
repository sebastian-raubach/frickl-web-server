...
import raubach.frickl.next.util.task.AlbumCountUpdateTask;
...
public class ImageScanner implements Runnable
{
	...
	private void setCreatedOnDirectory(DSLContext context, Path file)
	{
		...
		if (albumId != null)
		{
			AlbumsRecord newest = context.selectFrom(ALBUMS).where(ALBUMS.PARENT_ALBUM_ID.eq(albumId.getId())).orderBy(ALBUMS.CREATED_ON.asc()).fetchAny();

			if (newest != null)
			{
				Logger.getLogger("").info("SETTING CREATED ON FOR ALBUM: " + albumId.getName() + " -> " + newest.getName() + " " + newest.getCreatedOn());
