...
import raubach.frickl.next.util.ScriptRunner;
	...
	public static void init(String databaseServer, String databaseName, String databasePort, String username, String password, boolean initAndUpdate)
	{
		...
		if (initAndUpdate)
		{
			...
			if (!databaseExists)
			{
				...
				try
				{
					URL url = Database.class.getClassLoader().getResource("raubach/frickl/next/database/init/database.sql");
			...
			try
			{
				...
				Flyway flyway = Flyway.configure()
									  ...
									  .locations("classpath:raubach/frickl/next/database/migration")
			...
			try
			{
				URL url = Database.class.getClassLoader().getResource("raubach/frickl/next/database/init/views.sql");
